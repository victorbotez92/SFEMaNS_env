...
def fourier_to_phys(field_in,list_modes=None, mF_max=None, shift = 0.0): #requires "fourier" format (N, 2*D, MF), outputs "phys" format (N, D, 2*mF_max-1)
    """Slow version of inverse Fourier transform.
    Requirements: 
        numpy, einops
    Args:
        ...
        shift: float allowing for shift in theta in physical space
    Returns:
...
def phys_to_fourier(field_in, shift = 0.0): #requires "phys" format (N, D, 2*mF_max-1), outputs "fourier" format (N, 2*D, MF)
    """Slow version of inverse Fourier transform.
    Requirements: 
        numpy, einops
    Args:
